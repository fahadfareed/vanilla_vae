...
class Encoder(nn.Module):
    def __init__(
        ...
        self.convsigma = nn.Conv2d(in_channels=self.init_filters*(2**(self.n_depth-1)), out_channels=self.z_dim, kernel_size=self.kernel_size, stride=self.stride, padding=self.padding, bias=self.bias, groups = self.groups)
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.convrecon_mu = nn.Conv2d(in_channels=self.init_filters, out_channels=1, kernel_size=self.kernel_size, stride=self.stride, padding=self.padding, bias=self.bias, groups=self.groups)
        
        self.convrecon_logvar = nn.Conv2d(in_channels=self.init_filters, out_channels=1, kernel_size=self.kernel_size, stride=self.stride, padding=self.padding, bias=self.bias, groups=self.groups)
    ...
    def forward(self, x):
        ...
        recon = self.convrecon_mu(x)
        logvar_decoder = self.convrecon_logvar(x)
        return recon, logvar_decoder
...
class VAE(nn.Module):
    def __init__(
        ...
        """
        Creates a Denoising Variational Autoencoder model.
    ...
    def decode(self, z):
        ...
        """
        recon, logvar_decoder = self.decoder(z)
        out = (recon* self.data_std)+self.data_mean 
        return out
        
    def reparameterize(self, mu, sigma):
        """
        Uses reparametrization trick to draw a sample from a normal
        distribution given its mean and log variance.
        """
    ...
    def forward(self, x):
        ...
        return mu, sigma, recon, logvar_decoder
