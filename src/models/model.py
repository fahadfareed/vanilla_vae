...
class Encoder(nn.Module):
    def __init__(
        ...
        self.convmu = nn.Conv2d(
            in_channels=self.init_filters*(2**(self.n_depth-1)),
            out_channels=self.z_dim, kernel_size=self.kernel_size,
            stride=self.stride,
        ...
        for i in range(n_depth):
            ...
            down_conv = DownConv(in_channels=ins, out_channels = outs)
...
class Decoder(nn.Module):
    def __init__(
        ...
        for i in reversed(range(n_depth)):
            ...
            outs = self.init_filters*(2**i)
            up_conv = UpConv(in_channels=ins, out_channels=outs)
